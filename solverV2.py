...
def runGameLoop(GameMatrix, rows, cols) :
    ...
    while gameHasLegalMoves:
        ...
        if (len(currentMoveList) > 1) :
            indexNum = random.randint(0, len(currentMoveList) - 1)
            bestMove = currentMoveList[indexNum]
            bestMove.setMoveIndex(indexNum)
        else :
